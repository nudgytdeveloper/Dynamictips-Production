...
# These MUST match the labels the prompt tells the model to look for. They
# previously read "HUMAN"/"AI" while the prompt asked for the "HUMAN USER" and
# "AI CHARACTER" messages, so the model was directed at speaker labels that did
# not appear anywhere in the transcript. Staging already used these values.
FROM_TYPE_USER      = "User"
FROM_TYPE_CHARACTER = "Character"
SPEAKER_HUMAN       = "HUMAN USER"
SPEAKER_AI          = "AI CHARACTER"
...
# Note the upload UI shipped on staging first, so on production this is expected
# to be a no-op until the feature is promoted.
# ============================================================================
...
class DynamicTips:
    ...
    def combine_transcripts(self, transcripts: List[Dict[Any, Any]]) -> str:
        ...
        for transcript in transcripts:
            ...
            if from_type == FROM_TYPE_USER:
                speaker = SPEAKER_HUMAN
            elif from_type == FROM_TYPE_CHARACTER:
                speaker = SPEAKER_AI
            else:
                speaker = from_type
